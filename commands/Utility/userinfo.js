...
const status = {
 ...
 if (!game) {
  ...
  var authors = ["262410813254402048", "390884022173302805"]
      if (authors.includes(message.author.id && user.id)) {
                ...
                let rank;
                if (user.id === "262410813254402048") rank = "Bot Owner AND Owner of the server"
                if (user.id === "390884022173302805") rank = "Owner of the server"
        	let botuser;
  if (member.user.bot === true) botuser = 'Yes'
  else botuser = 'No'
	let embed = new Discord.RichEmbed()
	.setTitle(`__Userinfo On The ${rank}!__ :prince:`)
  .setColor("#ffd700")
...
message.channel.send({embed});
  ...
  .addField("***Username and Discriminator*** :name_badge:", `***>***__${user.tag}__`)
  .addField("***ID*** :id:", `***>***__${user.id}__`)
  .addField("***Created Account On*** :heavy_plus_sign:", `***>***__${dateformat(user.createdAt, "***mmmm dS, yyyy***, On a ***dddd***, ***h:MM:ss TT, Z***")}, that is **${Math.round(created.days())} day(s) ago!**__`)
  .addField(`***Status*** ${emoji}`, `***>***__${status[user.presence.status]}__`, true)
  .addField("***Last Message*** :speech_left:", `***>***__${lastmsg}__`, true)
  .addField(`***Joined ${message.guild.name} On*** :heavy_plus_sign:`, `***>***__${dateformat(member.joinedAt, "***mmmm dS, yyyy***, On a ***dddd***, ***h:MM:ss TT, Z***")}, that is **${Math.round(joined.days())} day(s) ago!**__`, true)
  .addField("***Playing*** :video_game:", `***>***__${game}__`, true)
  .addField("***Roles*** :scroll:", `***>***${roles.join(', ')}`, true)
  .addField("***Bot?*** :robot:", `***>***__${botuser}__`, true)
message.channel.send({embed});
